...
# KL DIVERGENCE
The Kullback-Leibler Divergence,or `KL Divergence` for short, is a measure of dissimilarity between two distributions. It can be interpreted as measuring the likelihood that samples represented by the empirical distribution $p$ were generated by a fixed distribution q. If $D_{KL} (q || p)=0$, we can guarantee that $p$ is generated by $q$. As $D_{KL} (q || p) \rightarrow \infty$, we can say that it is increasingly unlikely that $p$ was generated by $q$.
