...
$$ D_{KL} (p || q) = \mathbb{E}_{y_{k}} \left [ \log \left ( \frac{y_{k}}{q(y_{k})} \right ) \right ] = H(p, q) - H(p) = \sum_{k=1}^{K} p(y_{k}) \log\left[\frac{p(y_{k})}{q(y_{k})}\right] =\sum_{k=1}^{K} p(y_{k}) \left[\log p(y_{k}) - \log q(y_{k})\right]$$
